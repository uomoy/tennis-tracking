...
from TrackNet_light import TrackNet
...
# 	return K.mean(loss)
def custom_loss(y_true, y_pred):
    loss = (-1) * (K.square(1 - y_pred) * y_true * K.log(K.clip(y_pred, tf.keras.backend.epsilon(), 1)) +
                   K.square(y_pred) * (1 - y_true) * K.log(K.clip(1 - y_pred, tf.keras.backend.epsilon(), 1)))
    return K.mean(loss)

#Training for the first time
if paramCount['load_weights'] == 0:
	model=TrackNet(HEIGHT, WIDTH)
	ADADELTA = tf.keras.optimizers.Adam(learning_rate=1e-3) # 1에서 변경함
	model.compile(loss=custom_loss, optimizer=ADADELTA, metrics=['accuracy']) # tf.keras.metrics.Accuracy()
#Retraining
else:
...
plot_model(model , show_shapes=True , to_file='TrackNet.png')
model.summary()
...
for i in range(epochs):
	...
	with open("/home/porsche/dalhong/TrackNetV2/trained_models_tennis/" + str(i+1) + ".txt", 'w') as f:
		f.write("Outcome of training data of epoch " + str(i+1) + ":\n")
		f.write("Number of true positive: {}\n".format(TP))
		f.write("Number of true negative: {}\n".format(TN))
		f.write("Number of false positive FP1: {}\n".format(FP1))
		f.write("Number of false positive FP2: {}\n".format(FP2))
		f.write("Number of false negative: {}\n".format(FN))
	#Save intermediate weights during training
	if (i + 1) % 1 == 0:
		model.save(save_weights + str(i + 1))
